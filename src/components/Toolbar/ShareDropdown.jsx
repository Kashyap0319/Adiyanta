...
import { API_CONFIG, FRONTEND_CONFIG } from '../../config'
...
const ShareDropdown = ({ onClose, onUpgrade }) => {
  ...
  // Strip base64 image data from frames before sharing — keeps payload small
  const stripImagesFromFrames = (frames) => {
    ...
    return frames.map(frame => ({
      ...
      elements: Array.isArray(frame.elements)
        ...
        : frame.elements,
      ...(frame.backgroundImage && typeof frame.backgroundImage === 'string' && frame.backgroundImage.startsWith('data:')
        ? { backgroundImage: '' }
        : {})
    }))
  }
  ...
  const handleGetLink = async () => {
    try {
      ...
      const res = await fetch(`${API_CONFIG.baseURL}/share`, {
        method: 'POST',
      ...
      const shareUrl = `${FRONTEND_CONFIG.domain}/share/${result.shareId}`
