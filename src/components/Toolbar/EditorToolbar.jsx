...
const EditorToolbar = ({
  ...
  const toolbarItems = [
    ...
    { id: 'insert', label: 'Insert', isLabel: true },
    ...
    { id: 'tables', label: 'Tables', icon: 'table', action: onAddTable },
  ...
  return (
    <div className="flex items-center gap-1 px-3 py-1.5 border border-gray-200 rounded-full bg-white shadow-sm">
      {toolbarItems.map((item) => (
        <div key={item.id} className="relative">
          {item.isDivider ? (
            <div className="w-px h-6 bg-gray-200 mx-1" />
          ) : item.isLabel ? (
            <span className="text-sm text-gray-400 px-2">{item.label}</span>
          ) : (
            <button
              onClick={() => {
              ...
              title={item.title}
              className={`flex items-center gap-2 ${item.isIconOnly ? 'px-2' : 'px-3'} py-1.5 rounded-full text-sm font-medium transition-all ${
                item.disabled
