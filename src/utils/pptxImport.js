...
export async function parsePPTX(file) {
  ...
  for (const frame of frames) {
    ...
    // Light boundary enforcement — trust PPTX coordinates, only fix extremes
    for (const el of frame.elements) {
      // Ensure minimum size
      if (el.width < 5) el.width = 5
      if (el.height < 5) el.height = 5

      // Clamp font size to reasonable range
      if (el.type === 'text' && el.fontSize) {
        if (el.fontSize > 120) el.fontSize = 120
        if (el.fontSize < 4) el.fontSize = 4
      }
    }
...
async function parseElementsFromDoc(
  ...
  // Find the shape tree (<p:spTree>) which holds all slide content
  const spTree = doc.getElementsByTagName('p:spTree')[0]
  const topLevelNodes = spTree ? Array.from(spTree.childNodes) : []

  // Helper: get direct child elements by tag name (non-recursive)
  const getDirectChildren = (parent, tagName) => {
    const results = []
    for (let i = 0; i < parent.childNodes.length; i++) {
      const child = parent.childNodes[i]
      if (child.nodeName === tagName || child.localName === tagName.split(':').pop()) {
        results.push(child)
      }
    }
    return results
  ...
  // Parse direct p:sp children of spTree (not nested inside groups)
  const directSpNodes = spTree ? getDirectChildren(spTree, 'p:sp') : []
  for (let i = 0; i < directSpNodes.length; i++) {
    const el = await parseShapeElement(directSpNodes[i], ++elementId, emuToX, emuToY, emuToW, emuToH, zip, scaleFactor, slideBackgroundColor)
    if (el) {
      el.source = source
      elements.push(el)
    }
  ...
  // Parse direct p:pic children of spTree (not nested inside groups)
  const directPicNodes = spTree ? getDirectChildren(spTree, 'p:pic') : []
  for (let i = 0; i < directPicNodes.length; i++) {
    const el = await parseImageElement(directPicNodes[i], ++elementId, emuToX, emuToY, emuToW, emuToH, relsMap, zip)
    if (el) {
      el.source = source
      elements.push(el)
    }
  ...
  // Parse group shapes (<p:grpSp>) — flatten children with coordinate transformation
  const directGrpSpNodes = spTree ? getDirectChildren(spTree, 'p:grpSp') : []
  for (let i = 0; i < directGrpSpNodes.length; i++) {
    const grpSp = directGrpSpNodes[i]
    const result = await parseGroupShape(grpSp, elementId, emuToX, emuToY, emuToW, emuToH, relsMap, zip, scaleFactor, slideBackgroundColor, source)
    for (const el of result.elements) {
      elements.push(el)
    }
    elementId = result.nextElementId
  }

  // Parse table elements (<p:graphicFrame> containing <a:tbl>) — direct children only
  const gfNodes = spTree ? getDirectChildren(spTree, 'p:graphicFrame') : []
...
/**
 * Parse a <p:grpSp> (group shape) node.
 * Group shapes have their own coordinate space defined by:
 *   - a:off / a:ext on the group itself (position & size on the slide)
 *   - a:chOff / a:chExt (child coordinate space origin & extent)
 * Child elements use coordinates relative to chOff/chExt, which must be
 * transformed to slide coordinates:
 *   slide_x = grp_off_x + (child_off_x - chOff_x) * (grp_ext_cx / chExt_cx)
 */
async function parseGroupShape(grpSpNode, startId, emuToX, emuToY, emuToW, emuToH, relsMap, zip, scaleFactor, slideBackgroundColor, source) {
  const elements = []
  let elementId = startId

  // Get group transform
  const grpSpPr = grpSpNode.getElementsByTagName('p:grpSpPr')[0]
  ...
  if (grpSpPr) {
    ...
    if (xfrm) {
      ...
      if (chExt) {
        chExtCx = parseInt(chExt.getAttribute('cx')) || 1
        chExtCy = parseInt(chExt.getAttribute('cy')) || 1
      }
    }
  }

  // Scale factors for child → slide coordinate transformation (in EMU space)
  const scaleX = grpExtCx / chExtCx
  const scaleY = grpExtCy / chExtCy

  // Create wrapped EMU converters that apply group transform before converting to pixels
  const grpEmuToX = (emu) => emuToX(grpOffX + (emu - chOffX) * scaleX)
  const grpEmuToY = (emu) => emuToY(grpOffY + (emu - chOffY) * scaleY)
  const grpEmuToW = (emu) => emuToW(emu * scaleX)
  const grpEmuToH = (emu) => emuToH(emu * scaleY)
  ...
  // Parse direct p:sp children
  const childSpNodes = getDirectChildren(grpSpNode, 'p:sp')
  for (let i = 0; i < childSpNodes.length; i++) {
    const el = await parseShapeElement(childSpNodes[i], ++elementId, grpEmuToX, grpEmuToY, grpEmuToW, grpEmuToH, zip, scaleFactor, slideBackgroundColor)
    if (el) {
      el.source = source
      elements.push(el)
    }
  ...
  // Parse direct p:pic children
  const childPicNodes = getDirectChildren(grpSpNode, 'p:pic')
  for (let i = 0; i < childPicNodes.length; i++) {
    const el = await parseImageElement(childPicNodes[i], ++elementId, grpEmuToX, grpEmuToY, grpEmuToW, grpEmuToH, relsMap, zip)
    if (el) {
      el.source = source
      elements.push(el)
    }
  ...
  const nestedGroups = getDirectChildren(grpSpNode, 'p:grpSp')
  for (let i = 0; i < nestedGroups.length; i++) {
    const result = await parseGroupShape(nestedGroups[i], elementId, grpEmuToX, grpEmuToY, grpEmuToW, grpEmuToH, relsMap, zip, scaleFactor, slideBackgroundColor, source)
    for (const el of result.elements) {
      elements.push(el)
    }
    elementId = result.nextElementId
