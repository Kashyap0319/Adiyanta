...
const createDefaultFrame = (id, title = 'New Frame') => ({
  id,
  title,
  preview: title,
  elements: [
...
const createBlankProject = () => ({
  ...
  frames: Array.from({ length: 10 }, (_, i) => createBlankFrame(i + 1, `Slide ${i + 1}`)),
})
...
export const EditorProvider = ({ children }) => {
  ...
  */
  ...
  const addFrame = useCallback(() => {
    ...
    const newId = maxId + 1
    const newFrame = createDefaultFrame(newId, `Slide ${frames.length + 1}`)
