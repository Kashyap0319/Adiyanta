...
import { topics, licenses, sortOptions, formatDownloads, getLicenseDisplay } from '../../utils/templateData'
...
const HomePage = () => {
  ...
  const topicOptions = useMemo(() => {
    ...
    const configSubjects = Array.isArray(config?.subjects) ? config.subjects : []
    const templateTopics = templates.map(template => template?.topic)

    const mergedTopics = [...topics, ...configTopics, ...configSubjects, ...templateTopics]
      .map(topic => typeof topic === 'string' ? topic.trim() : '')
      .filter(Boolean)

    const uniqueTopics = mergedTopics.reduce((acc, topic) => {
      const normalizedTopic = topic.toLowerCase()
      if (normalizedTopic === 'all') return acc
      if (!acc.some(existingTopic => existingTopic.toLowerCase() === normalizedTopic)) {
        acc.push(topic)
      }
      return acc
    }, [])
  ...
  }, [config?.topics, config?.subjects, templates])
