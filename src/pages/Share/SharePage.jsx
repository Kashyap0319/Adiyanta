...
const SharePage = () => {
  ...
  useEffect(() => {
    ...
    const loadPresentation = async () => {
      try {
        const res = await fetch(`${API_CONFIG.baseURL}/share/${shareId}`)
        ...
        if (result.success && result.data) {
          ...
          setError(result.message || 'Presentation not found or link has expired.')
        }
      } catch (loadError) {
        logger.error('Failed to open share link:', loadError)
        if (!cancelled) setError('Unable to load this shared presentation. Please check the link.')
      } finally {
